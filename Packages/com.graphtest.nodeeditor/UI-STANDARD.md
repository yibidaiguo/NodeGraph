...
| 场景 | 用法 |
|---|---|
| 节点视觉 | 框架 `node-base` 全家。**节点只有一种轮廓：圆角矩形**，角色由**整宽实色标题色带**（`--ne-node-title-fill`，上圆下方，`BeginTitleBarPath`）+ 色带上的**语义图标**表达；色带文字与图标统一取 `--ne-node-title-text`。标题行固定 34px 最小高度，图标是 16px 扁平 glyph，**无金属底座**。领域节点只用 `[NodeIcon]` 注册 `NodeIconKind`，不得带位图/SVG 或自建皮肤，未注册时才按 Role 回退。原生 `Node/#node-border` 矩形背景和边框在 normal/hover/selected 全部保持透明。running/success/failure 必须改变整个轮廓的填充、上沿高光、下沿阴影、描边与克制辉光；selected 只叠加向外偏移 3px 的独立陶土色轮廓环，不得替换运行态填充；validation error/warn 走独立内轮廓，不能露出矩形选择框。颜色必须来自 `var(--ne-*)`，命中区域使用同一组圆角曲线采样。领域**不得**自定义节点皮肤 |
...
所有节点同为圆角矩形，轮廓不承载角色信息。`NodeIconKind` 决定标题色带上画哪个语义 glyph。领域节点只用 `[NodeIcon]` 注册 `NodeIconKind`，不得带位图/SVG 或自建皮肤；未注册时按 Role 回退。

节点表面是单层扁平：底色 + 标题色带 + 细描边，不绘制顶点色渐变网格。运行态流光画在色带之上、所有轮廓之下（`DrawRunningFlow`）。Painter2D 的几何属性必须带单位，写成裸 `0` 会导致整条标题色带不绘制。
