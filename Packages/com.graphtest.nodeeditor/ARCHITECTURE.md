...
16. **代码结构固定，项目资源位置由用户配置**：① 产品代码在各 asmdef 的 `Runtime/` / `Editor/` 根下按职责分文件夹；移动 `.cs` 必须保留 `.meta` / GUID。② 节点定义、图、黑板、数据库、本地化与注册表的实际落点不是架构常量；每个生成/装载入口都从唯一的项目级 `NodeEditorAssetPaths` 或领域 `<Domain>AssetPaths` 读取。Setup 只播种 `Assets/NodeGraph/<Module>/...` 一类可编辑默认值，用户可改成任意规范化、项目自有的 `Assets/` 子目录；改默认值不会自动搬动或覆盖现有作者资产。③ 配置资产首次引导到 `Assets/NodeGraph/Settings/`，但 locator 按类型在项目 `Assets/` 中解析；缺失时带 Undo 创建，重复时失败并列出候选，绝不静默取第一个。领域配置不得持有框架全局路径，框架配置不得认识领域目录。④ 产品与集成测试从当前配置解析资源；只有明确创建/销毁自身临时资产的隔离测试可使用测试目录常量。范本见 [Runtime/Data/NodeEditorAssetPaths.cs](Runtime/Data/NodeEditorAssetPaths.cs)、[Editor/Support/NodeEditorAssetPathsLocator.cs](Editor/Support/NodeEditorAssetPathsLocator.cs) 与 [ProjectAssetPaths.cs](Editor/Support/ProjectAssetPaths.cs)。
...
- **NodeEditor 拥有 UI 标准（2026-07-09；完整规范见 [`UI-STANDARD.md`](UI-STANDARD.md)——扩展 UI 前必读，含组件目录/铁律/扩展清单，契约测试强制执行）**：`NodeEditorStyles.uss` 定义跨模块 tokens 与 shared classes（toolbar command/text/icon/toggle、`node-cue`、badge/chip、form row、banner、hover tooltip、data 三列状态）；`EditorUi` 暴露这些 class 常量和小型 helper；`NodeCueControl` 负责节点 cue 的标签、刷新、截断和两行稳定化。Graphite Premium Tool palette 保留深色画布和黑钛/石墨编辑面板，用统一深色节点标题、4px 角色窄脊线、细描边、选中描边与少量语义 accent 改善可读性，避免大面积高亮、彩色标题块或糖果色节点。窗口顶栏一律 `AppBar`（三区：在哪 / 留白 / 看什么），随需面板走画布浮层 `OverlayPanel`，临时面板走自绘 `Popover`；**命令按钮**（`ne-appbar-cmd`、图切换器胶囊 `ne-picker-pill`、toolbar 文本/图标按钮、添加/删除按钮、组合框箭头、hover-bar 按钮）必须走共享 premium button tokens（底色/hover/active/disabled/描边/高光/阴影）；**导航 chrome**（顶栏导航钮 `ne-appbar-nav`、面板开关 `ne-panel-seg-btn`、档位页签 `ne-tabs/-tab`、画布坞 `ne-canvas-dock-btn`、弹层行 `ne-popover-row`、历史条 `breadcrumb-crumb/-sep`、折叠箭头 `collapsible-arrow`、数据窗口左列 `data-scope-title`+`data-source-row`、图切换器行 `ne-picker-row`）走扁平导航契约——透明底、hover 软底、选中强调色软底 + 常驻透明脊线占位（选中不位移），契约由 `NodeEditorStyles_UsesFlatNavigationChrome` 锁定；领域模块不得自建平面黑矩形或亮蓝按钮皮肤，也不得把导航行重新做成凸起按钮。Dialogue/Task 等领域只继承/消费这些 contracts，不复制 USS token 或自建平行控件。本地化文本统一用 `EditorUi.CurrentLanguageTextRow` 的语言下拉 + 全宽高文本区，候选来自 `LanguageOptions`，禁止回退到语言 chip + 窄文本框或多语言挤在同一区。普通视觉颜色必须在 USS；C# 直接设色只允许 Unity API 例外（当前明确例外：GraphView `Port.portColor`）。所有可见 UI 文案、校验、报错走 `Localizer.UI`，CJK/长 key 通过共享 dropdown/detail row contract 保持不挤出、不丢值。
...
- `[NodeIcon(typeof(MyNode), NodeIconKind.X)]` / `NodeIconRegistry` 是节点语义图标的唯一领域接缝：领域在自己的 Editor 注册 carrier 类上声明节点类型到稳定语义 kind 的映射，避免 Runtime 程序集依赖编辑器 UI；框架 `NodeIconControl` 用 Painter2D 绘制共享线图标和金属底座，未声明类型按 `NodeRole` 回退。位图、SVG、颜色、尺寸、阴影和主题不得进入领域包；这些全部归 NodeEditor 的控件与 USS token 所有。
...
## 为什么 NodeIconKind 是封闭枚举

`NodeIconKind` 列举了各领域的词汇（Dialogue / Choice / Task / Gate / State / Transition …），
看上去违反「加新模块不用改框架」：每引入一个新语义都要回框架加枚举值。
这一点被反复提出，记在这里以免重复论证。

**结论：保持封闭。**

节点扁平化之后，所有节点同为圆角矩形，形状不再区分角色——**角色信息全部压在标题色带的颜色和那个
16px 语义 glyph 上**。图标从"辅助识别"变成了"唯一的角色信号"，框架对这套词汇的掌控因此比以前更要紧，
而不是更松。

开放成「字符串 id + 领域自带绘制器」会带来两件事：

1. **画笔交给领域。** glyph 线宽、尺寸、亮暗主题取色都是框架 token，集中实现才守得住。
   三个模块各画各的，跨模块一致性就只剩文档约束，而文档约束是守不住的。
2. **语义可以无限增殖。** 封闭枚举强制"新语义要先想清楚它和已有的哪个重合"，
   开放注册表则让每个模块顺手造一个自己的，最终同义不同形。

代价是真实的：新增语义确实要改框架（枚举 + 绘制两处）。接受它，是因为图标语义增长是低频的，
而视觉一致性一旦破防就收不回来。

判据：**模块之间理应长得不一样的东西该模块化；必须长得一样的该留在框架。**
节点定义、创建配方、外壳文案、退役样例清单属于前者，都已模块化；图标词汇属于后者。

重新考虑的时机：出现本仓库之外的第三方模块，且确有框架未覆盖的语义。
届时的正解仍不是开放绘制器，而是扩充枚举，或提供一组通用 Kind 供其复用。
