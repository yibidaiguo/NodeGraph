...
- `[NodeIcon(NodeIconKind)]` / `NodeIconRegistry` 是节点语义图标的唯一领域接缝：领域只在自己的 `NodeDefinition` 类型上声明稳定的语义 kind；框架 `NodeIconControl` 用 Painter2D 绘制共享线图标和金属底座，未声明类型按 `NodeRole` 回退。位图、SVG、颜色、尺寸、阴影和主题不得进入领域包；这些全部归 NodeEditor 的控件与 USS token 所有。
