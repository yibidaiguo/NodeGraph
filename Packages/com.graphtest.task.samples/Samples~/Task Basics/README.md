...
```
Assets/Samples/NodeGraph Task Samples/0.0.1/Task Basics/
├─ Data/
...
从 NodeGraph Manager 导入后，Unity 会把样例复制到 `Assets/Samples/NodeGraph Task Samples/0.0.1/Task Basics`。移除样例包不会自动删除这个已导入副本；不再需要时请手动删除该目录。
